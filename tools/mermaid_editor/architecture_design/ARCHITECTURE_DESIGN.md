...
- 当前已提供基础 `Ctrl+Z` 撤销，但尚未实现重做与系统化历史管理
- 当前 PNG 导出还不是浏览器画布级别的高保真截图

当前已知风险：

- Web 画布的交互复杂度继续上升后，需要更明确的前后端状态同步策略
- `editor.html` 当前单文件规模较大，状态、渲染、交互和 bridge 调用耦合较高，不利于持续维护
...
状态：已完成，已补齐重做（`Ctrl+Y` / `Ctrl+Shift+Z`）、边端点侧向编辑与 `Web -> Qt` 运行时消息边界；当前仍保留 `Qt -> Web` 具名 JS 调用作为兼容下行链路，但不构成 M4 阻塞
...
12. 在 v0.1.3 中补充左侧 `1/3` 默认占比与最大 `1/2` 限制、工具栏单行填充、基础撤销与更稳的子图拖拽链路
13. Mermaid 解析改为“两阶段解析”：先收集定义，再解析边，以修复“边先于定义出现”时的误读
14. 已补充嵌套子图 `direction` 默认布局与“边先于定义出现”的回归测试
15. 当前已建立 `to_agent/AGENT_TASK_DISPATCH.md`，作为后续框架重构与多 Agent 交接中心文档
...
- 在保持现有技术路线的前提下，下一阶段优先做“分层重构”而不是“重写技术栈”

当前仍属于后续演进项的内容：

- 更完整的 preview / action 能力
- 更强的 Mermaid 语法兼容
- 更完整的编辑器能力，如重做、更细的边样式控制、高保真导出和系统化历史管理
