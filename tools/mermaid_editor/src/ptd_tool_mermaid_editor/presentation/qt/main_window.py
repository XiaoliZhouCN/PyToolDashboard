...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.layout_store = LayoutRepository()
    ...
    def _build_ui(self) -> None:
        ...
        left_splitter = QSplitter(Qt.Orientation.Vertical, left_panel)
        self.left_splitter = left_splitter
        left_layout.addWidget(left_splitter)

        upper_panel = QWidget(left_splitter)
        upper_layout = QVBoxLayout(upper_panel)
        upper_layout.setContentsMargins(0, 0, 0, 0)
        upper_splitter = QSplitter(Qt.Orientation.Horizontal, upper_panel)
        self.upper_splitter = upper_splitter
        upper_layout.addWidget(upper_splitter)

        navigator_panel = QWidget(upper_splitter)
        ...
        detail_panel = QWidget(upper_splitter)
        ...
        self.detail_edge_source_anchor_combo = QComboBox(form_widget)
        self.detail_edge_source_anchor_combo.addItems(["auto", "top", "right", "bottom", "left"])
        self.detail_edge_target_anchor_combo = QComboBox(form_widget)
        self.detail_edge_target_anchor_combo.addItems(["auto", "top", "right", "bottom", "left"])
        ...
        form_layout.addRow("Edge Source Side", self.detail_edge_source_anchor_combo)
        form_layout.addRow("Edge Target Side", self.detail_edge_target_anchor_combo)
        ...
        upper_splitter.addWidget(navigator_panel)
        upper_splitter.addWidget(detail_panel)
        upper_splitter.setStretchFactor(0, 4)
        upper_splitter.setStretchFactor(1, 3)

        source_panel = QWidget(left_splitter)
        source_layout = QVBoxLayout(source_panel)
        source_layout.setContentsMargins(0, 0, 0, 0)
        self.source_editor = QPlainTextEdit(source_panel)
        self.source_editor.setPlaceholderText("Selected Mermaid source appears here.")
        source_layout.addWidget(self.source_editor)

        left_splitter.addWidget(upper_panel)
        left_splitter.addWidget(source_panel)
        ...
        splitter.splitterMoved.connect(self._clamp_main_splitter)

        self._update_main_splitter_bounds()
    ...
    def apply_source_changes(self) -> None:
        ...
        if self.current_path:
            self.layout_store.restore_diagram(
                self.current_path,
                parsed,
    ...
    def on_selection_changed(self, payload: str) -> None:
        ...
        self.detail_summary.setText(self._build_detail_summary(selection))
        ...
        self.detail_edge_source_anchor_combo.setVisible(kind == "edge")
        self.detail_edge_target_anchor_combo.setVisible(kind == "edge")
        ...
        self.detail_edge_source_anchor_combo.setCurrentText(
            str(selection.get("source_anchor_side", "auto"))
        )
        self.detail_edge_target_anchor_combo.setCurrentText(
            str(selection.get("target_anchor_side", "auto"))
        )
    ...
    def apply_detail_changes(self) -> None:
        ...
        payload = {
            ...
            "source_anchor_side": self.detail_edge_source_anchor_combo.currentText(),
            "target_anchor_side": self.detail_edge_target_anchor_combo.currentText(),
    ...
    def _set_detail_enabled(self, enabled: bool) -> None:
        ...
        self.detail_edge_source_anchor_combo.setEnabled(enabled)
        self.detail_edge_target_anchor_combo.setEnabled(enabled)
    ...
    def _clear_detail_fields(self) -> None:
        ...
        self.detail_edge_source_anchor_combo.setCurrentText("auto")
        self.detail_edge_target_anchor_combo.setCurrentText("auto")
        self.detail_anchor_combo.setCurrentText("auto")

    @staticmethod
    def _build_detail_summary(selection: dict[str, Any]) -> str:
        kind = str(selection.get("kind", "none"))
        if kind == "subgraph":
            return str(selection.get("label") or selection.get("id") or "")
        if kind == "node":
            return f"Selected node: {selection.get('id', '')}"
        if kind == "edge":
            source = str(selection.get("source", "")).strip()
            target = str(selection.get("target", "")).strip()
            edge_title = f"{source} -> {target}".strip(" ->")
            return f"Selected edge: {edge_title or selection.get('id', '')}"
        return f"Selected {kind}: {selection.get('id', '')}"

    @classmethod
    def _bounded_left_width(cls, total_width: int, left_width: int) -> int:
        if total_width <= 0:
            return 0
        min_left = int(total_width * cls.MIN_LEFT_PANEL_RATIO)
        max_left = int(total_width * cls.MAX_LEFT_PANEL_RATIO)
        if max_left < min_left:
            max_left = min_left
        return max(min_left, min(max_left, left_width))

    def _update_main_splitter_bounds(self) -> None:
        total_width = max(self.main_splitter.width(), self.width(), sum(self.main_splitter.sizes()), 1)
        min_left = int(total_width * self.MIN_LEFT_PANEL_RATIO)
        max_left = int(total_width * self.MAX_LEFT_PANEL_RATIO)
        if max_left < min_left:
            max_left = min_left
    ...
    def _apply_initial_splitter_sizes(self) -> None:
        ...
        left_width = self._bounded_left_width(
            total_width=total_width,
            left_width=int(total_width * self.DEFAULT_LEFT_PANEL_RATIO),
        )
        self._update_main_splitter_bounds()
        self.main_splitter.setSizes([left_width, total_width - left_width])

        total_left_height = max(self.height(), 900)
        self.left_splitter.setSizes([int(total_left_height / 3), int(total_left_height * 2 / 3)])

    def _clamp_main_splitter(self, *_args) -> None:
        sizes = self.main_splitter.sizes()
        if len(sizes) != 2:
            return
        total = sizes[0] + sizes[1]
        bounded_left = self._bounded_left_width(total_width=total, left_width=sizes[0])
        if sizes[0] != bounded_left:
            self.main_splitter.blockSignals(True)
            self.main_splitter.setSizes([bounded_left, total - bounded_left])
