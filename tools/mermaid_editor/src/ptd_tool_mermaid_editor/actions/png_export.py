...
class PngExporter:
    ...
    def _draw_edge(self, painter: QPainter, diagram: MermaidDiagram, edge: EdgeModel) -> None:
        ...
        path.cubicTo(QPointF(x1 + 60, y1), QPointF(x2 - 60, y2), QPointF(x2, y2))
        painter.drawPath(path)

        if edge.style != "plain":
            self._draw_arrowhead(painter, x2, y2)
    ...
    def _draw_arrowhead(self, painter: QPainter, x: float, y: float) -> None:
