...
class DiagramEditor:
    ...
    VALID_ANCHOR_SIDES = {"auto", "top", "right", "bottom", "left"}
    ...
    def normalize(self, diagram: MermaidDiagram) -> MermaidDiagram:
        ...
        for node in diagram.nodes:
            ...
            if node.anchor_side not in self.VALID_ANCHOR_SIDES:
                node.anchor_side = "auto"
        ...
        for subgraph in diagram.subgraphs:
            if subgraph.parent_subgraph == subgraph.subgraph_id:
                ...
                subgraph.parent_subgraph = None
            if subgraph.anchor_side not in self.VALID_ANCHOR_SIDES:
                subgraph.anchor_side = "auto"
        ...
        for edge in diagram.edges:
            if edge.source_anchor_side not in self.VALID_ANCHOR_SIDES:
                edge.source_anchor_side = "auto"
            if edge.target_anchor_side not in self.VALID_ANCHOR_SIDES:
                edge.target_anchor_side = "auto"
        return diagram
