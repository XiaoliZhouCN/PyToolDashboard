...
class LayoutRepository:
    ...
    def apply(self, diagrams: list[MermaidDiagram], layout_data: dict[str, Any]) -> None:
        for diagram in diagrams:
            self.apply_to_diagram(diagram, layout_data)
    ...
    def apply_to_diagram(
        ...
        *,
        ...
        """Apply persisted layout to a single diagram, keeping parser defaults for new items."""

        diagram_layout = self._diagram_layout(
            layout_data,
            diagram_id=diagram_id or diagram.diagram_id,
        )
    ...
    def restore_diagram(
        ...
        *,
        ...
        """Load sidecar data from disk and apply it to a reparsed diagram."""

        self.apply_to_diagram(
            diagram,
            self.load(markdown_path),
            diagram_id=diagram_id or diagram.diagram_id,
        )
        return diagram
    ...
    @staticmethod
    def _diagram_layout(layout_data: dict[str, Any], *, diagram_id: str) -> dict[str, Any]:
        diagrams_layout = layout_data.get("diagrams")
        if isinstance(diagrams_layout, dict):
            return diagrams_layout.get(diagram_id, {})
        return layout_data


__all__ = ["LayoutRepository"]
